...
\usepackage{float}
\usepackage{graphicx} % Adicione noo preâmbulo, se ainda não estiver lá
...
% Tabela ou lista com prazos e metas, linkando com as etapas descritas acima.
A Tabela \ref{tab:cronograma_projeto} apresenta o cronograma do projeto, detalhando as etapas e os períodos estimados para cada uma delas. O cronograma foi elaborado considerando a complexidade de cada etapa e a necessidade de ajustes finos durante o desenvolvimento.

\begin{table}[H]
 \begin{center}
  \caption{Cronograma do projeto de classificação de lixo}
  \label{tab:cronograma_projeto}
  \begin{tabular}{c|c}
  \hline
  \textbf{Etapa} & \textbf{Período} \\
  \hline
    Coleta de dados & 01/08 a 15/08 \\
    Pré-processamento & 16/08 a 25/08 \\
    Desenvolvimento e treinamento do modelo & 26/08 a 15/09 \\
    Escolha da plataforma de hardware & 26/08 a 15/09 \\
    Quantização do modelo & 16/09 a 22/09 \\
    Validação e ajuste fino & 23/09 a 06/10 \\
    Implantação & 07/10 a 20/10 \\
    Avaliação dos resultados & 21/10 a 27/10 \\
    Escrita do Trabalho de Conclusão II & 01/08 a 31/10 \\
  \hline
  \end{tabular}
 \end{center}
 {\sourcecitation{\textcite{Do autor}}}
...
Após a conclusão do trabalho, espera-se obter um modelo de aprendizado de máquina embarcado capaz de classificar imagens de lixo em 12 categorias diferentes com acurácia e eficiência compatíveis com soluções atuais. O modelo deverá ser executável em um microcontrolador com recursos limitados, demonstrando a eficácia da aplicação de técnicas TinyML em dispositivos embarcados. Ademais, comparativos entre a solução TinyML e aplicações de inteligência artificial tradicionais deverão evidenciar as vantagens do processamento local, como redução de latência, menor consumo energético e independência de conectividade com a nuvem, comparando as métricas de desempenho e consumo de recursos computacionais.
