...
Para o cumprimento do objetivo específico 4 será utilizada uma Rede Neural Convolucional (RNC); a escolha concreta da arquitetura (MobileNetV2) é detalhada adiante no Capítulo \ref{cap:desenvolvimento}, Seção \ref{sec:treinamento}.
...
Ao longo deste capútlo e dos próximos, serçao citados trechos de código que podem ser consultados pelo leitor em: \href{https://github.com/maxwellrs-dev/ai4rpi}{github.com/maxwellrs-dev/ai4rpi}.
...
A utilização do conjunto de dados RealWaste neste trabalho permitiu uma comparação direta com os resultados apresentados no estudo que introduziu o dataset \cite{single2023realwaste}. No artigo, cinco arquiteturas de redes neurais convolucionais — VGG-16, InceptionResNetV2, DenseNet121, InceptionV3 e MobileNetV2 — foram avaliadas sobre o mesmo conjunto de imagens. O melhor desempenho foi obtido pela arquitetura InceptionV3, com acurácia de 89,19\%, precisão de 91,34\%, sensibilidade de 87,73\% e F1-score de 90,25\%.

No presente trabalho, utilizando uma MobileNetV2 quantizada em INT8, foi alcançada uma acurácia e F1-score globais próximos de 0,68 no conjunto de validação, com tempos médios de inferência da ordem de 200~ms no dispositivo embarcado. Embora inferiores aos valores relatados no estudo original, essa diferença é esperada e pode ser justificada por aspectos metodológicos e restrições do ambiente embarcado.

Em primeiro lugar, o estudo de referência empregou imagens em resolução de 524\,$\times$\,524~px, além de um processo de aumento de dados mais agressivo, que triplicou o tamanho do conjunto de treinamento utilizando transformações geométricas complexas (como \emph{elastic distortion} e \emph{shear}). Neste trabalho, optou-se por uma resolução menor (224\,$\times$\,224~px) e um conjunto mais restrito de transformações, de modo a equilibrar qualidade do treinamento e limitações de memória e processamento do sistema embarcado. Essa diferença reduz o espaço de atributos aprendido pelo modelo e, consequentemente, sua capacidade discriminativa.

Além disso, os modelos do estudo original incluem arquiteturas substancialmente mais complexas, com profundidade e quantidade de parâmetros superiores à MobileNetV2 utilizada aqui, especialmente após a quantização para INT8. Redes como InceptionV3 e DenseNet121 apresentam maior capacidade de separação entre classes visualmente semelhantes — como vidro, metal e plástico — que também foram justamente as classes onde mais se observou confusão neste trabalho.

Por outro lado, apesar do desempenho mais modesto em acurácia, a solução desenvolvida aqui demonstra a viabilidade prática da classificação de resíduos sólidos diretamente em hardware de baixo custo. Enquanto o estudo de referência avaliou o desempenho apenas em ambiente computacional tradicional, sem restrições de hardware, este trabalho valida que um modelo compactado pode operar em tempo quase real, com baixo consumo de CPU e memória, mantendo estabilidade durante toda a execução.

Em síntese, os resultados obtidos não indicam inconsistência metodológica, mas refletem os compromissos exigidos pelo cenário embarcado. O estudo de referência demonstra o limite superior do desempenho possível com arquiteturas robustas e sem restrições computacionais, enquanto este trabalho evidencia a aplicabilidade prática do aprendizado de máquina embarcado em dispositivos de recursos limitados. Assim, ambas as abordagens se complementam: enquanto a primeira investiga desempenho máximo, a segunda valida a operação real em contexto TinyML.
...
Os experimentos realizados demonstram que o uso de TinyML é uma alternativa viável para sistemas que exigem processamento local, baixa latência e independência de conectividade. Embora o desempenho obtido seja inferior ao de modelos executados em ambientes tradicionais de maior capacidade computacional, os resultados mostram que soluções compactas e quantizadas podem operar de forma robusta e eficiente em dispositivos de baixo custo.
...
Em síntese, o trabalho confirma que técnicas de TinyML podem substituir ou complementar arquiteturas tradicionais de IoT em aplicações práticas, consolidando o potencial da computação de borda como elemento fundamental na evolução de sistemas inteligentes embarcados.

Assim, além de comprovar a viabilidade técnica, este estudo evidencia que
aplicações TinyML representam um caminho concreto para a próxima geração de
sistemas inteligentes embarcados, reforçando o papel de soluções compactas e
eficientes na arquitetura tecnológica contemporânea.
