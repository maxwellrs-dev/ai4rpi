...
O estudo de caso foi desenvolvido em etapas, baseada no fluxo de trabalho apresentado na Seção \ref{sec:implantacao} com a adição de alguns pontos. As etapas são apresentadas a seguir com suas respectivas atribuições:
...
\begin{enumerate}
    ...
    Nesta etapa, foi implementada uma Rede Neural Convolucional (RNC) utilizando TensorFlow. Essa arquitetura é ideal para classificação de imagens, permitindo que o modelo aprenda padrões visuais complexos necessários para identificar diferentes categorias de lixo.
...
\label{sec:hardware}
Nesta seção é apresentada a justificativa da escolha da plataforma de execução do modelo e a preparação realizada para torná-la apta à inferência embarcada. A escolha pela \textbf{Raspberry Pi 3 Model B+} foi definida com base em um conjunto de critérios técnicos que a qualificam como uma plataforma ideal para a validação de soluções TinyML em ambiente de borda, oferecendo o \textbf{equilíbrio ideal entre capacidade e restrição}.
