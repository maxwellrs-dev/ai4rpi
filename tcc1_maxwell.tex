...
Todo o procedimento está implementado em \texttt{metadados/aumento\_de\_dados.py}, que mantém a estrutura de diretórios por classe, grava o conjunto balanceado em disco e facilita a reprodução dos resultados.
    	...
    	extbf{Justificativa da simplicidade.} Optou-se por um pipeline enxuto porque:
\begin{itemize}
...
Esta seção descreve a implementação prática do modelo de classificação, desde a preparação do pipeline de dados até o treinamento com ajuste fino. A implementação foi conduzida em ambiente de nuvem do Google Colab, com aceleração disponível, utilizando TensorFlow/Keras. Estas escolhas foram feitas com base principalmente na facilidade de acesso, onde basta ter um navegador da web para acessar a plataforma da Google. Uma vez dentro do ambiente, é possível utilizar CPUs, GPUs e TPUs gratuitamente (com limitações) e instalar bibliotecas, pacotes e dependências conforme necessário. Além disso, o Colab oferece integração nativa com o Google Drive, facilitando o armazenamento e o compartilhamento de arquivos.

\subsection{Pipeline de dados}
Os conjuntos de treino e validação foram criados a partir do diretório base do \textit{RealWaste}, empregando o TensorFlow, com divisão estratificada aproximada de 80/20 e semente 123 para reprodutibilidade. As imagens foram redimensionadas para 224\(\times\)224 px e carregadas em lotes (\textit{batch}) de 64 amostras.

\subsection{Arquitetura e estratégia de treino}
Utilizou-se a \textbf{MobileNetV2} como extratora de atributos base (\texttt{include\_top = false}, \texttt{weights = imagenet}, \texttt{alpha = 1}), por ser uma arquitetura leve, adequada ao contexto TinyML. Sobre a base, adicionaram-se camadas finais para classificação multiclasse:
...
A estratégia de treinamento adotou duas fases:
\begin{enumerate}
    \item \textbf{Aquecimento (\textit{warm-up})}: base congelada (\texttt{base\_model.trainable = False}) por 5 épocas, para estabilizar as camadas adicionadas;
    \item \textbf{Ajuste fino (\textit{fine-tuning})}: base liberada parcialmente, mantendo congeladas todas as camadas exceto as \textbf{80 últimas}, permitindo especialização progressiva dos filtros superiores à tarefa.
\end{enumerate}

\subsection{Função de perda, otimizador e \textit{callbacks}}
O modelo foi compilado com \texttt{loss = sparse\_categorical\_crossentropy} e métrica principal \texttt{accuracy}. Como otimizador, empregou-se \textbf{AdamW} com taxa de aprendizado inicial de \(1\times 10^{-4}\) e \textit{weight decay} \(1\times 10^{-5}\).

Para robustez adicional frente a eventuais desequilíbrios residuais das classes, foram usados \textbf{pesos por classe} (\textit{class weights}) obtidos via \texttt{sklearn.utils.compute\_class\_weight} sobre as \textit{labels} do conjunto de treino.

Dois \textit{callbacks} controlaram o processo de otimização:
\begin{itemize}
    \item \textbf{\textit{EarlyStopping}} monitorando \texttt{val\_loss}, com \texttt{patience = 10} e restauração dos melhores pesos;
    \item \textbf{\textit{ReduceLROnPlateau}} (\texttt{factor = 0{,}5}, \texttt{patience = 3}, \texttt{min\_lr = 1e-6}), reduzindo a taxa de aprendizado quando a validação estagna.
\end{itemize}

O treino completo foi limitado a \textbf{150 épocas} e interrompido pelo \textit{early stopping} quando apropriado.

\subsection{Resultados do treino}
Durante o treinamento, foram salvas as curvas de \textbf{acurácia} e \textbf{perda} para treino e validação. A Figura~\ref{fig:treino_curvas} exibe essas curvas ao longo das épocas, evidenciando a convergência do modelo e o ponto de parada determinado pelo \textit{early stopping}.
...
Ao final, o modelo em ponto flutuante (\texttt{.keras}) foi salvo para posterior conversão e quantização (Seção~\ref{sec:quantizacao}).
