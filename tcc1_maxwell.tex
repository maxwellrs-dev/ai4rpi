...
\begin{figure}[H]
    ...
    \sourcecitation{\cite{realwaste2023}}
\end{figure}
...
Todo o procedimento está implementado em \texttt{metadados/aumento\_de\_dados.py}, que mantém a estrutura de diretórios por classe, grava o conjunto balanceado em disco e facilita a reprodução dos resultados.
    	...
    	extbf{Justificativa da simplicidade.} Optou-se por um fluxo enxuto porque:
\begin{itemize}
    \item o dataset já era limpo e uniformizado (522\(\times\)522 px), de modo que etapas adicionais de filtragem/segmentação não agregariam benefício claro;
    \item em cenários TinyML, evitar pré-processamentos pesados reduz acoplamento e mantém a inferência simples no dispositivo embarcado;
    \item as transformações escolhidas aumentam a diversidade sem introduzir artefatos ou deslocar o domínio dos dados, favorecendo a generalização em condições reais (iluminação e enquadramento variáveis);
    \item manter o fluxo curto melhora a reprodutibilidade e reduz o risco de \textit{data leakage} entre treino e teste.
\end{itemize}

Por fim, a normalização de intensidade (por exemplo, reescala para [0,1] ou padronização por canal) é tratada no pipeline de treinamento, junto com a alimentação em lotes, não exigindo transformações adicionais na etapa de pré-processamento offline.
...
O arquivo quantizado foi carregado via \texttt{tf.lite.Interpreter}. O processo de avaliação seguiu:
\begin{itemize}
    \item Alocação de tensores e leitura de metadados de entrada/saída;
    \item Conversão condicional das imagens para \texttt{uint8} conforme tipo exigido pelo tensor de entrada;
    \item Execução de inferência (\texttt{invoke()}) e coleta das distribuições de probabilidades para cada classe;
    \item Cálculo de acurácia, precisão ponderada, sensibilidade ponderada, F1 ponderado, matriz de confusão normalizada e curvas ROC multiclasse a partir das pontuações.
\end{itemize}

\subsection{Principais benefícios e trade-offs}
\begin{itemize}
    \item \textbf{Redução de tamanho}: modelos INT8 tipicamente ocupam ~25\% do espaço do equivalente em ponto flutuante, neste estudo tivemos uma redução de 10,4 Mb para 2,7 Mb no modelo quantizado;
    \item \textbf{Latência menor}: operações inteiras são mais rápidas em hardware embarcado quando comparadas ao mesmo modelo em ponto flutuante, resultando em inferências mais ágeis;
    \item \textbf{Energia}: menor número de ciclos computacionais reduz consumo energético por inferência;
    \item \textbf{Impacto na acurácia}: pequena queda potencial mitigada por um conjunto representativo bem escolhido; neste trabalho, a calibração usou amostras diversificadas dos lotes iniciais do treino;
    \item \textbf{Compatibilidade}: entrada/saída em \texttt{uint8} simplifica implantação em microcontroladores que carecem de FPU (sigla em inglês para Unidade de Ponto Flutuante).
\end{itemize}
...
\label{sec:hardware}
Nesta seção é apresentada a justificativa da escolha da plataforma de execução do modelo e a preparação realizada para torná-la apta à inferência embarcada. Optou-se por utilizar uma \textbf{Raspberry Pi 3 Model B+} principalmente pela \textit{facilidade de acesso}: já se encontrava disponível fisicamente no início do projeto, evitando custos adicionais e atrasos de aquisição. A escolha conserva também benefícios técnicos relevantes para um protótipo TinyML:

\begin{itemize}
    \item Equilíbrio entre \textbf{recursos computacionais} (CPU quad-core 64-bit) e \textbf{simplicidade de uso};
    \item Comunidade ampla e maturidade de \textbf{ecossistema de software} (suporte direto a Python, TensorFlow Lite e bibliotecas de acesso a periféricos);
    \item \textbf{Custo e disponibilidade} inferiores a placas mais avançadas (ex.: Jetson), mantendo desempenho suficiente para um modelo convolucional quantizado;
    \item Suporte a \textbf{câmera} para aquisição direta de imagens;
\end{itemize}
...
\begin{table}[H]
    ...
    \hline
    \end{tabular}
    {\sourcecitation{Ficha técnica do fabricante}}
\end{table}
...
\begin{enumerate}
    \item \textbf{Gravação do sistema operacional}: uso do Raspberry Pi Imager para instalar Raspberry Pi OS Lite (versão 64-bit), reduzindo consumo de recursos e tempo de inicialização;
...
\subsection{Adequação ao escopo do projeto}
Para o modelo quantizado em INT8, o hardware oferece margem confortável de CPU e RAM, permitindo incluir instrumentação de métricas (registro de latência e uso de memória) sem comprometer a responsividade. Embora microcontroladores menores representem cenários TinyML mais restritos, a Raspberry Pi 3 B+ facilita fases exploratórias de prototipagem antes de um eventual porte para plataformas com menos recursos.

Em síntese, a escolha foi guiada por disponibilidade imediata, suporte amplo de software e equilíbrio entre recursos e simplicidade, viabilizando medições confiáveis do impacto da quantização.
...
\subsection{Observações e melhorias possíveis}
\begin{itemize}
    \item \textbf{Normalização}: Se o modelo original foi treinado com normalização para [0,1] ou centragem em [-1,1], seria necessário aplicar a mesma transformação antes da quantização ou adaptar a calibração; neste caso o conjunto representativo em uint8 permite uso direto.
    \item \textbf{Mapeamento de classes}: A lista \texttt{CLASS\_NAMES} deve refletir exatamente a ordem interna do modelo; caso contrário a interpretação semântica da saída estará incorreta.
    \item \textbf{Latência}: A medição simples pode ser refinada repetindo inferências consecutivas e descartando a primeira (\textit{warm-up}).
    \item \textbf{Robustez}: Uma captura contínua (loop) permitiria comparar desempenho médio (FPS) e aplicar filtros de estabilização luminosa.
    \item \textbf{Uso de memória}: Ferramentas como \texttt{/usr/bin/time -v} ou leitura de \texttt{/proc/meminfo} podem ser integradas para registrar consumo real durante testes.
    \item \textbf{Tratamento de iluminação}: Ajustes automáticos simples (balanceamento de branco ou equalização de histograma) podem melhorar consistência em ambientes variáveis; evitou-se aqui para manter simplicidade.
\end{itemize}

\subsection{Resultado funcional}
O teste básico produziu saída textual contendo a classe de maior probabilidade e a latência observada, confirmando que o modelo quantizado executa com responsividade adequada na Raspberry Pi 3 B+. Este resultado demonstra a viabilidade do fluxo TinyML completo: coleta e preparação (Seção \ref{sec:preprocessamento}), treinamento (Seção \ref{sec:treinamento}), quantização (Seção \ref{sec:quantizacao}) e execução embarcada (esta seção).
...
% Fallback para subchapter caso não exista no template
\providecommand{\subchapter}[1]{\section{#1}}

\subchapter{Avaliação de desempenho}
\label{sec:avaliacao-desempenho}

Esta seção sumariza o desempenho do classificador com base no roteiro executado no notebook \texttt{metadados/cria\_modelo.ipynb}, avaliando o \textbf{modelo quantizado em INT8 (TFLite)} sobre o conjunto de validação. São reportadas métricas globais (médias ponderadas), a matriz de confusão normalizada por classe e as curvas ROC multiclasse.
...
\begin{itemize}
    ...
    \item \textbf{Sensibilidade ponderada} (SEN/Rec.): média ponderada do \textit{recall} por classe, ponderada pelo suporte;
    \item \textbf{F1 ponderado}: média ponderada do F1 por classe, combinando precisão e sensibilidade.
\end{itemize}

Os resultados consolidados para o conjunto de validação foram:
...
\begin{table}[H]
 ...
 \begin{tabular}{l c}
  ...
  Sensibilidade/Recall (ponderado) & 0,68 \\
...
\subsection{Curvas ROC multiclasse}
A Figura~\ref{fig:roc_multiclasse} mostra as \textbf{curvas ROC por classe} e as respectivas áreas sob a curva (AUC), calculadas a partir dos escores de saída do modelo para cada rótulo. Em cenários multiclasse, cada curva contrasta o desempenho \textit{um-versus-rest} de uma classe específica.
...
\begin{figure}[H]
    ...
    \fbox{\parbox{0.85\textwidth}{\centering Placeholder: inserir em \\
    	exttt{imagens/roc\_multiclasse.png} as curvas ROC por classe.}}%
    }
    \caption{Curvas ROC por classe e respectivas AUCs.}
    \label{fig:roc_multiclasse}
    {\sourcecitation{Do autor}}
\end{figure}

\subsection{Discussão}
Em linhas gerais, as médias ponderadas (PRE, SEN, F1) refletem o impacto do \textit{suporte} de cada classe; assim, classes com menos amostras tendem a apresentar maior variância nas métricas individuais, influenciando menos o agregado ponderado. A leitura conjunta da matriz de confusão e das curvas ROC permite identificar pares de classes mais propensos à confusão e orientar possíveis ajustes: coleta adicional para classes minoritárias, refinamento de aumento de dados direcionado e, se necessário, afrouxar ou ajustar limiares de decisão em cenários operacionais específicos.
